"""
DuckDB connector for PandasAI.

Custom SQLConnector that allows PandasAI to use DuckDB
as the SQL execution backend.
"""
...
class DuckDBConnector(SQLConnector):
    """
    PandasAI connector for DuckDB tables.

    Wraps a DuckDB connection to work with PandasAI's Agent.
    Multiple connectors share the same connection ID so PandasAI
    treats them as one datasource (required for direct_sql mode).
    """
    ...
    def __init__(self, connection, table_name: str, field_descriptions: dict = None):
        ...
        # Required by SQLConnector - skip parent __init__
    ...
    def equals(self, other) -> bool:
        """Check if connectors share the same datasource."""
